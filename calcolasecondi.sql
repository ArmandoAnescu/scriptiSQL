...
datediff (now(), t.data_assunzione ) mod 365 div 30 as mesi, 
datediff (now(), t.data_assunzione ) mod 365 mod 30 as giorni,
-- SECOND(datediff (now(), t.data_assunzione ) ) as secondi,
...
-- il codice fiscale non si ripete mai 
select * from amministrazione.personale; 

-- vincolo di attributo/ unique (codice fiscale)
-- modifichiamo la struttura della tabella personale 
 alter table amministrazione.personale
 modify column codice_fiscale unique;  
